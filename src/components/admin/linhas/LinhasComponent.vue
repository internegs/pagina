        ...
        >
            ...
            <table class="table table-striped table-hover mb-4">
                ...
                <tbody>
                    ...
                    >
                        <td>
                            {{ linha?.nome ?? 'Não definido' }}
                            <br />
                        </td>

                        <td>
                            <span>{{ maskNumber(linha.number) }}</span>
                        </td>
...
import CreateOrEditModal from './modals/linhas-modal/CreateOrEditModal.vue'
    ...
    methods: {
        async getLinhas() {
            ...
            Api.post(
                ...
                .then(response => {
                    this.linhas = response.data.dados.data
        ...
        async getSubplanos() {
            Api.post('/subplanos', {
                ...
                .then(response => {
                    this.subplanos = response.data.subplanos.map(subplano => ({
                        id: subplano.id,
                        nome: subplano.nome,
                        plano_id: subplano.plano_id,
                    }))
                })
                .catch(err => {
                    console.log(err)
        ...
        async Deleta(linha_id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    Api.post(`/config_delete`, {
                        Y2xpZW50ZV9pZA: this.CLIENTE_ID,
                        Y29uZmlnX2RlbGV0ZQ: btoa(linha_id),
                    })
                        .then(() => {
                            Swal.fire({
                                position: 'center',
                                icon: 'success',
                                title: 'Sucesso!',
                                text: 'Linha deletada com sucesso.',
                        ...
                        .catch(error => {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: 'Erro ',
                                titleText: 'Erro ao deletar linha.',
                                didOpen: () => {
                                    const confirmBtn = Swal.getConfirmButton()
                                    const actionsContainer = confirmBtn.parentElement
